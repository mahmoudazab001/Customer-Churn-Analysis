...
--## Conclusion
--1) customer demographics affect the churn rate by a small amount.
--2) causes that affect customers churn are :
		--1 customers with month to month contract type.
		--2 customers with contract lenth less that a year.
		--3 customers that isn't in a group
		--4 customers with int calls/ mins but has no int plan.
		--5 customers with more that 2 customers calls have higher tendancy to leave.
		--6 70% of customer left has no Device Protection & Online Backup 
--3 by analysing churn category and churn reasons the previous causes were confermed to be reliable and true.